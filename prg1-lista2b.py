...
def decompor_numero(numero):
    ...
    '''
    if (numero < 1000):
        centena = numero // 100 #retorna 3  
        valor = numero % 100 #retorna 26
        dezena = valor // 10
        unidade = valor % 10
...
def caixa_eletronico(valor):
    ...
    if (valor <= 600):
        ...
        valor = valor %100
        if notas_100 > 0:
            notas.append((100,notas_100))
        
        notas_50 =valor//50
        valor = valor % 50
        if notas_50 > 0:
            notas.append((50,notas_50))
        
        notas_25 =valor//25
        valor = valor % 25
        if notas_25 > 0:
            notas.append((25,notas_25))
        
        notas_10 =valor//10
        valor = valor % 10
        if notas_10 > 0:
            notas.append((10,notas_10))
        
        notas_5 =valor//5
        valor = valor % 5
        if notas_5 > 0:
            notas.append((5,notas_5))
        
        notas_1 = valor
        if notas_1 > 0:
            notas.append((1,notas_1))
        return notas"""
        #outra maneira de fazer
        """notas = []
        
        for nota in (100,50,25,10,5,1):
            qt_notas = valor // nota
            valor = valor % nota
            if qt_notas > 0:
                notas.append((nota, qt_notas))
        return notas"""
        #terceira maneira de fazer
        
        notas = []
        for nota in (100,50,25,10,5,1):
            qt_notas,valor  = divmod(valor,nota)
            if qt_notas > 0:
                notas.append((nota, qt_notas))
        return notas    
