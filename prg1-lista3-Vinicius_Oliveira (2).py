...
# Exercícios sem while, apenas com for e sem funções embutidas.
...
def palindrome(texto):
	...
	for x in texto:
		if x in string.punctuation:
			new_texto = texto.replace(x,'')
		else:
			new_texto = texto
	if new_texto == new_texto[::-1]:
		return True
	else:
		return False
...
def imprime_mes_por_extenso(data):
	...
	'''
	meses = 'janeiro favereiro março abril maio junho julho agosto setembro outubro novembro dezembro'.split()
...
def é_sortudo(numero):
	...
	return "2" in str(numero) and "7"  not in str(numero)
...
def é_número_válido(numero):
	...
	if (é_azarado(numero) is False) and (soma_é_par(numero) is True) and (é_chato(numero) is False):
		return True
	else:
		return False
